    ...
    if (joinState === 'idle') {
        return (
            <div className={`h-screen flex flex-col items-center justify-center font-sans ${isDark ? 'bg-gray-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
                ...
                <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Requesting camera and microphone access.</p>
            </div>
        );
    }
        ...
        <div className={`h-screen flex flex-col overflow-hidden font-sans selection:bg-blue-500/30 transition-colors duration-300 ${isDark ? 'bg-[#202124]' : 'bg-gray-100'}`}>
            ...
            <div className={`absolute bottom-0 left-0 right-0 h-20 ${isDark ? 'bg-[#202124]' : 'bg-white border-t border-gray-200'} flex items-center justify-between px-6 z-40 transition-colors duration-300`}>
                ...
                <div className={`flex items-center space-x-4 font-medium text-sm hidden md:flex ${isDark ? 'text-white' : 'text-gray-800'}`}>
                    ...
                    <span>{roomId}</span>
                    <div className="flex items-center space-x-1 pl-2 border-l border-gray-400/30">
                        <button
                            onClick={copyRoomId}
                            className={`p-1.5 rounded transition ${isDark ? 'hover:bg-white/10 text-gray-300' : 'hover:bg-black/5 text-gray-600'}`}
                            title="Copy Room ID"
                        >
                        ...
                        <a
                            ...
                            className={`p-1.5 rounded transition ${isDark ? 'hover:bg-white/10 text-green-400' : 'hover:bg-black/5 text-green-600'}`}
